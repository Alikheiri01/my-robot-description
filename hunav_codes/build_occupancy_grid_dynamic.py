...
- PEDESTRIAN_EXCLUSION_RADIUS is a single hardcoded constant (0.5m) rather
  than a per-agent CLI radius -- now lives in hunav_config.py alongside
  every other tunable constant, matching (with a small safety margin) the
  known agent radius (0.4m) in thesis_static_agent.yaml. Still not read
  from that YAML directly, so it must be updated by hand if the scenario's
  agent radius ever changes -- but at least no longer duplicated across files.

RESOLVED -- SPAWN_X/SPAWN_Y/SPAWN_YAW used to be duplicated here AND in
...
# PEDESTRIAN_EXCLUSION_RADIUS, odom_to_world() and yaw_from_quaternion() now
